...
def format_sample(question: dict) -> None:
    ...
    st.markdown(f"> **{blue_background}**")
    st.markdown(f"> {remove_punctuation(question["context_after"])}")
    st.markdown("")
    st.markdown("")
    st.markdown(":grey-background[Now read the modified text which omits the highlighted part:]")
    st.markdown("")
    st.markdown("")
    st.markdown(f"> {remove_punctuation(question["context_before"])}  > **{question["sentence_1"]}**  > {remove_punctuation(question["context_after"])}")
    st.markdown("")
    st.markdown("")
